...
- Production provisions exactly one 512 MB Fly Machine in `lhr` and uses a pooled Neon Postgres `DATABASE_URL` in AWS London (`eu-west-2`). Fly stops that Machine after several idle minutes and auto-starts the same Machine on the next request, so zero Machines may be running while one remains provisioned. Presence, reconnect reservations, and per-room queues are process-local, so do not provision more than one Machine until cross-instance Socket.IO fan-out, distributed presence/reservations, command serialization, and multi-instance tests exist.
