...
Changes are limited to styling classes in `MultiplayerStatus`, `GameView`, and `GameHeader`, plus focused regression assertions. Multiplayer state, protocol behavior, puzzle layout calculations, and control behavior are unchanged.
...
- Update focused component tests to assert the compact multiplayer card, shared game-grid classes, and reduced header top padding.
